export const ADMIN_EMAIL = "mov30444@gmail.com";
export const ADMIN_PASS  = "admin1234";

export const SPECIALTIES = [
  "Cardiology","Neurology","General Medicine","Dermatology",
  "Pediatrics","Orthopedics","Psychiatry","Gynecology",
  "Ophthalmology","ENT","Urology","Oncology",
  "Dentistry", "Physiotherapy", "Gastroenterology", "Endocrinology", "Pulmonology",
  "Nephrology", "Radiology", "Homeopathy", "Ayurveda", "Dietician",
  "Rheumatology", "Hematology", "Infectious Disease", "Plastic Surgery", "Neurosurgery",
];

export const DEGREES = [
  "MBBS","MD","MS","BDS","MDS","MBBS + MD","MBBS + MS","DM","MCh",
];

export const SLOT_CATEGORIES = {
  morning: ["08:00 AM", "08:30 AM", "09:00 AM", "09:30 AM", "10:00 AM", "10:30 AM", "11:00 AM", "11:30 AM"],
  afternoon: ["12:00 PM", "12:30 PM", "01:00 PM", "01:30 PM", "02:00 PM", "02:30 PM", "03:00 PM", "03:30 PM"],
  evening: ["04:00 PM", "04:30 PM", "05:00 PM", "05:30 PM", "06:00 PM", "06:30 PM", "07:00 PM", "07:30 PM"],
  night: ["08:00 PM", "08:30 PM", "09:00 PM", "09:30 PM", "10:00 PM", "10:30 PM"]
};




